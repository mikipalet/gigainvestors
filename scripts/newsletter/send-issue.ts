...
async function main() {
  ...
  if (testTo) {
    const r = await resend.emails.send({ from, to: testTo, subject: `[test] ${manifest.subject}`, html: html.replace("{{{RESEND_UNSUBSCRIBE_URL}}}", "https://gigainvestors.com/newsletter"), tags: [{ name: "issue", value: slug }] });
